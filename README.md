# Amicron – Community-Dokumentation

[![DeepWiki](https://img.shields.io/badge/DeepWiki-satwareAG%2Fafmodocs-blue.svg?logo=data:image/png;base64,)](https://deepwiki.com/satwareAG/afmodocs)

**Community-Dokumentation für Amicron-Faktura und Amicron-Mailoffice – gepflegt von der [satware® Amicron Business Solutions Community](https://www.facebook.com/groups/amicron)**

> 🤖 Dieses Repository ist für KI-gestützte Dokumentationstools optimiert (DeepWiki, [satware® AI chat](https://chat.satware.ai/)) und dient als „Amicron Almanach" für Support-Teams.

## Über Amicron

[Amicron Software](https://amicron.de/) entwickelt seit 1991 (gegründet von Andreas Kleine) führende Lösungen für kleine und mittlere Unternehmen im deutschsprachigen Raum. Die Produktfamilie umfasst zwei sich ergänzende Anwendungen:

### Amicron-Faktura

**Warenwirtschaft und Auftragsbearbeitung** – Die Kernlösung für:
- Kunden-, Lieferanten- und Artikelverwaltung
- Angebote, Aufträge, Lieferscheine, Rechnungen
- Mahnwesen und offene Posten
- E-Rechnung (ZUGFeRD/XRechnung)
- Optionales Fibu-Modul mit DATEV-Schnittstelle und Elster
- SQL-Datenbank (Firebird), netzwerkfähig

### Amicron-Mailoffice

**Kontakt- und E-Mail-Management** – Die Ergänzung für:
- Vollständiger E-Mail-Client (POP3/IMAP/SMTP), netzwerk- und teamfähig
- Kalender mit Team-Funktionen und CalDAV-Integration
- Aufgabenverwaltung mit Wiedervorlage
- Werbeaktionen und Serienbriefe/-E-Mails
- Telefonie (TAPI/CAPI) mit Anruferkennung
- Outlook-Synchronisation (Professional)

**Integration**: Beim gemeinsamen Einsatz teilen beide Anwendungen dieselbe Datenbank für Adressen und Kontakte – ein integriertes Paket für Auftragsbearbeitung, Kontakt-/E-Mail-Management und Terminplanung.

### satware AG als Vertriebspartner

Die **[satware AG](https://satware.com)** ist langjähriger Vertriebspartner von Amicron Software und setzt selbst seit der DOS-Version **Topfakt 2.0** (Anfang der 1990er Jahre) auf Amicron-Produkte. Mit über 30 Jahren praktischer Erfahrung im täglichen Einsatz bringen wir tiefgehendes Fachwissen in diese Community-Dokumentation ein.

## Zweck

Dieses Repository bietet:

1. **Original-Dokumentation** – Archivierte Hersteller-Dokumentation (`docs/vendor-reference/`)
2. **Community-Erweiterungen** – Korrekturen, Tutorials, Tipps von Community-Mitgliedern (`docs/community/`)
3. **Technische Referenz** – Undokumentierte Features, Entity-Links, Datenbankschema (`docs/reference/`)
4. **Support-FAQs** – Themenspezifische FAQ-Dateien für KI-gestützten Support (`docs/faq/`)

## Schnellzugriff

| Bereich | Beschreibung |
|---------|--------------|
| [Erste Schritte](docs/getting-started/overview.md) | Einführung in Amicron |
| [Referenz](docs/reference/) | Technische Referenzmaterialien |
| [FAQs](docs/faq/) | Häufig gestellte Fragen nach Themen |
| [Fehlerbehebung](docs/troubleshooting/) | Bekannte Probleme und Lösungen |
| [Community](docs/community/) | Tutorials und Tipps von Community-Mitgliedern |

## Mitwirken

Siehe [CONTRIBUTING.md](CONTRIBUTING.md) für Richtlinien zum Hinzufügen von Dokumentation.

**Community-Mitglieder**: Eure Support-Tickets und Lösungen können zu FAQ-Einträgen werden!

## KI-Integration

Diese Dokumentation wird indexiert von:
- **[DeepWiki](https://deepwiki.com/satwareAG/afmodocs)** – KI-gestütztes Wiki (wöchentliche Aktualisierung)
- **[satware® AI chat](https://chat.satware.ai/)** – KI-gestützter Support mit RAG-Integration

## Lizenz

[![CC BY 4.0](https://licensebuttons.net/l/by/4.0/88x31.png)](https://creativecommons.org/licenses/by/4.0/)

Diese Community-Dokumentation steht unter der [Creative Commons Attribution 4.0 International (CC BY 4.0)](https://creativecommons.org/licenses/by/4.0/) Lizenz.

**⚠️ Wichtig:** Dies ist **inoffizielle Community-Dokumentation**. Für rechtliche Hinweise, Markenrechte und Haftungsausschlüsse siehe [LEGAL.md](LEGAL.md).

© 2026 satware AG und satware® Amicron Business Solutions Community Mitwirkende.

---

*Gepflegt von [satware AG](https://satware.com) • Kontakt: mw@satware.com*
